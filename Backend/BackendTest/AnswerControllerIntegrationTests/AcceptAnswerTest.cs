...
public class AcceptAnswerTest(ITestOutputHelper outputHelper) : Tester(outputHelper)
{
    ...
    public async Task CanOnlyAcceptAnAnswerIfQuestionHasNoAcceptedAnswers()
    {
        ...
        var postAns2Res = await AnsHelper.PostNewAnswerToQuestion(q.Id,
            new NewAnswer { Content = "testing", PostedAt = DateTime.Now }, token);
    ...
    public async Task AdminCanAcceptAnyAnswer()
    {
        ...
        var postRes = await QHelper.PostQuestion(
            new NewQuestion { Title = "test", Content = "test", PostedAt = DateTime.Now, Tags = [] },
            user.SessionToken);
        postRes.EnsureSuccessStatusCode();
        var q = await postRes.Content.ReadFromJsonAsync<QuestionDTO>();
        Assert.NotNull(q);
        ...
        var postAnsRes = await AnsHelper.PostNewAnswerToQuestion(q.Id,
            new NewAnswer { Content = "tester", PostedAt = DateTime.Now }, user.SessionToken);
        postAnsRes.EnsureSuccessStatusCode();
        var ans = await postAnsRes.Content.ReadFromJsonAsync<AnswerDTO>();
        Assert.NotNull(ans);
