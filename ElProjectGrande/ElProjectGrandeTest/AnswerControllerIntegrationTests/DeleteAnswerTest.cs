...
public class DeleteAnswerTest(ITestOutputHelper outputHelper) : Tester(outputHelper)
{
    ...
    public async Task DeleteAnswer()
    {
        ...
        var postQRes = await QHelper.PostQuestion(new NewQuestion { Title = "title", Content = "content", PostedAt = DateTime.Now, Tags = []}, token);
        postQRes.EnsureSuccessStatusCode();
        var q = await postQRes.Content.ReadFromJsonAsync<QuestionDTO>();
        Assert.NotNull(q);

        var postAnsRes = await AnsHelper.PostNewAnswerToQuestion(q.Id, new NewAnswer { Content = "content", PostedAt = DateTime.Now}, token);
    ...
    public async Task AdminCanDeleteAnyAnswer()
    {
        ...
        var postQRes = await QHelper.PostQuestion(new NewQuestion { Title = "title", Content = "content", PostedAt = DateTime.Now, Tags = []}, user.SessionToken);
        postQRes.EnsureSuccessStatusCode();
        var q = await postQRes.Content.ReadFromJsonAsync<QuestionDTO>();
        Assert.NotNull(q);

        var postAnsRes = await AnsHelper.PostNewAnswerToQuestion(q.Id, new NewAnswer { Content = "content", PostedAt = DateTime.Now}, user.SessionToken);
        postAnsRes.EnsureSuccessStatusCode();
        var ans = await postAnsRes.Content.ReadFromJsonAsync<AnswerDTO>();
        Assert.NotNull(ans);
