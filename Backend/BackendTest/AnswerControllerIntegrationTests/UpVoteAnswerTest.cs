...
//Need to add karma checking
public class UpVoteAnswerTest(ITestOutputHelper outputHelper) : Tester(outputHelper)
{
    [Fact]
    public async Task UpvoteAddsOneVoteAnswer()
    {
        ...
        var postRes =
            await QHelper.PostQuestion(
                new NewQuestion { Title = "test", Content = "test", PostedAt = DateTime.Now, Tags = [] }, token);
        ...
        var postAnsRes = await AnsHelper.PostNewAnswerToQuestion(q.Id,
            new NewAnswer { Content = "tester", PostedAt = DateTime.Now }, token);
    ...
    [Fact]
    public async Task DownvotedAnswerGetsUpvotedAnswerAddsTwo()
    {
        var loginRes = await UHelper.Login("admin@admin.com", "admin123");
        loginRes.EnsureSuccessStatusCode();
        var token = await loginRes.Content.ReadFromJsonAsync<string>();
    ...
    [Fact]
    public async Task UpvotingWhenAlreadyUpvotedUnvotesVoteAnswer()
    {
        var loginRes = await UHelper.Login("admin@admin.com", "admin123");
        loginRes.EnsureSuccessStatusCode();
        var token = await loginRes.Content.ReadFromJsonAsync<string>();
        ...
        var upvoteRes2 = await AnsHelper.UpvoteAnswer(ans.Id, token);
        upvoteRes2.EnsureSuccessStatusCode();
    ...
    public async Task CanOnlyVoteOnExistingAnswers()
    {
        ...
        var upvoteRes = await AnsHelper.UpvoteAnswer(Guid.NewGuid(), token);
    ...
    {
        var upvoteRes = await AnsHelper.UpvoteAnswer(Guid.NewGuid(), "");
