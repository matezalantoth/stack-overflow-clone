    ...
    : IUserRepository
{
    ...
    public bool IsUserLoggedIn(string username)
    {
        return (context.Users.FirstOrDefault(u => u.UserName == username) ??
                throw new NotFoundException("This user could not be found")).SessionToken != string.Empty;
    }
    ...
    public async ValueTask<User?> GetUserOnlyAnswers(string username)
    {
        return await context.Users
            .Include(u => u.Answers)
            .FirstOrDefaultAsync(u => u.UserName == username);
    ...
    public async ValueTask<User> MuteUserByUsername(string username, int mutedFor)
    {
        ...
        if (user == null) throw new NotFoundException("This user could not be found");
        user.Muted = true;
        user.MutedUntil = user.MutedUntil == null
            ? DateTime.Now.AddHours(mutedFor)
            : user.MutedUntil?.AddHours(mutedFor);
        await userManager.UpdateAsync(user);
        return user;
    }
    ...
    public async ValueTask<User?> GetUserOnlyQuestions(string username)
    {
        return await context.Users
            .Include(u => u.Questions)
            .ThenInclude(q => q.Tags)
